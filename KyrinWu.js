...
function checkBan(checkban) {
		...
		if (existsSync('/home/runner/.miraigban')) {
				...
				logger(global.getText('mirai', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
						...
						else return axios.get('https://raw.githubusercontent.com/Ryy00/Pogi/main/listban.json').then(_0x2f978e => {
		...
		return axios.get('https://raw.githubusercontent.com/Ryy00/Pogi/main/data.json').then(dataGban => {
...
loginApiData.setOptions(global.config.FCAOption)
				...
				logger(`C𝗹𝗶𝗳𝗳`, '[ C𝗹𝗶𝗳𝗳 ]');
				logger('Hey, thank you for using this Bot', '[ Cliff vincent ]');
				logger("Fixed by Cliff", '[ Cliff vincent ]');
				logger("FB:https://www.facebook.com/swordigo.swordslush ", 'Fixed by Cliff');
			var cron = require("node-cron");
			//Change credits pa bobo
const momentt = require("moment-timezone").tz("Asia/Manila");
		const day = momentt.day();
		const time = momentt.format("HH:mm:ss");
loginApiData.sendMessage(`𝘽𝙤𝙩 𝙝𝙖𝙨 𝙗𝙚𝙚𝙣 𝙖𝙘𝙩𝙞𝙫𝙖𝙩𝙚𝙙 𝙖𝙩 𝙩𝙞𝙢𝙚 ${time}`, global.config.ADMINBOT[0])
